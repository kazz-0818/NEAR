...
**`google_sheets_query` に送るのは次だけ:** ユーザーが **Googleスプレッドシート上の表を実際に読み取って** 数値・一覧・集計・所感を答えてほしいことが明らかなとき（メッセージに `docs.google.com/spreadsheets/d/...` がある、または会話の流れで「さっきのシート／この一覧／表データ」と特定のブックを見る必要が読み取れるとき）。**「表」が比喩だけ**、**Excelの使い方の一般質問**、**会話にシートが無いのに漠然と集計と言っているだけ**のときは `google_sheets_query` にしない。**迷ったら `simple_question`**（シート連携はオーケストレーション側の補助もある）。
