...
-- 1) 旧名のまま public に残っている場合のみリネーム（045 と同順・IF EXISTS で冪等）
...
-- 2) public に残った near_* を near スキhema へ（既に near にあれば何もしない）
...
COMMENT ON SCHEMA near IS 'NEAR アプリ専用テーブル（Veliora OS / AI 部署 near）。Table Editor では本スキーマを展開して確認してください。';
