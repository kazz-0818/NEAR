...
export function isUserConfusionOrNegationSignal(text: string): boolean {
  ...
  // 短い感嘆・疑問（<=10文字）
  if ([...t].length <= 10) {
    if (/^(は[?？!！\s,、。]*|え[?？!！\s,、。]*|ん[?？!！\s,、。]*|はぁ[?？!！\s]*|は[あ]+|なんで[?？]?|意味不明|わからん|わからない|違う[!！]?|なってない|おかしい)$/u.test(t)) {
      return true;
    }
  }
  // 混乱・疑問シグナル（長短問わず）
  if (
    /(どういうこと[?？,、。]*|どゆこと[?？,、。]*|どうゆうこと[?？,、。]*|どういう意味[?？]?|意味わからん|意味不明|ぜんぜんあかん|全然あかん|何の話[?？]?|話がズレ|ズレてる|ちがう|違う|そういうことじゃない|そうじゃなくて|それじゃない|なんかおかしい|文脈見て|ちゃんと汲み取って|なぜ番号|なんで番号|番号じゃない)/u.test(t)
  ) {
    return true;
  }
  // 対話・会話の失敗指摘
  if (
    /(会話(に|が)(なっていない|なってない|成立していない|できていない|できてない)|話が通じない|通じていない|通じてない|ズレた返答|的外れ|的を外れ|全然(違う|ちがう|ダメ|だめ)|全然関係ない|関係ない返答)/u.test(t)
  ) {
    return true;
  }
