      ...
      messages: [
        {
          ...
          content:
            ...
            "リアルタイム・地点依存が強い話題（天気・災害・為替など）は、(1) 学習データの限界を一言、(2) **確実な確認先**（気象庁・公式アプリ等）を短く、(3) 一般論・注意点があれば添える。**断定は避けつつ、製品全体を「未実装」と決めつけない**。**任意で最後に1文**、NEAR はまだ伸びしろがあるので、この LINE で「こうしてほしい」「使いにくい」を送ってもらえると改善の参考になる、という**前向きなお願い**を足してよい（長くしない・押し売りにしない・`成長パイプライン` などの内部用語は出さない）。\n" +
