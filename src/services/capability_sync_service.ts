...
/**
 * 成長完了時に near_capability_registry に1行追加（将来の DB 駆動 capabilities の足がかり）
 */
export async function registerCapabilityFromGrowth(input: {
  ...
  try {
    const ex = await input.db.query(`SELECT id FROM near_capability_registry WHERE intent_name = $1 LIMIT 1`, [
      intentSlug,
    ]);
    if (ex.rows.length > 0) {
      await input.db.query(
        `UPDATE near_capability_registry
      ...
      await input.db.query(
        `INSERT INTO near_capability_registry (intent_name, description, user_visible_line, enabled, sort_order)
