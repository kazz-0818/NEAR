...
export async function composeNearReply(input: ComposeInput): Promise<string> {
  ...
  try {
    ...
    if (sheetsSuccess) {
      userBits.push(
        "",
        "【スプレッドシート参照成功】ドラフトは**すでにシートを読み取り、集計・一覧化した結果**です。「リンクを開けない」「中身を覗けない」など**アクセス不能の断りは禁止**（矛盾する）。**見出し・箇条書きの構造と数字は維持**（段落作文に直しすぎない）。導入は短く、**締めの1文だけ** NEAR 口調（従順・軽口）に整えてよい。サービス定型の長いお礼は避ける。"
      );
    }
